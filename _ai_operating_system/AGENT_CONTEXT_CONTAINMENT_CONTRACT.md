...
**Each repository has exactly one source of truth for agent context: its own local `AGENTS.md`, `_ai_operating_system/`, and tool-specific adapter folders.**
...
- `_ai_operating_system/INSTRUCTION_PRECEDENCE_CONTRACT.md` — Load order resolution.
- `_ai_operating_system/REPO_OPERATING_PROFILE.md` — Repo-specific operating mode.
- `_ai_operating_system/PROJECT_PROFILE.md` — Project identity and scope.
- `_ai_operating_system/MASTER_SYSTEM_PROMPT.md` — Meta-system doctrine.
- `_ai_operating_system/EXECUTION_PROTOCOL.md` — Execution discipline.
- `_ai_operating_system/VALIDATION_GATES.md` — Validation discipline.
- All other files in `_ai_operating_system/` directory.
...
### Category 4: Runtime Application Code (MUST NOT depend on `_ai_operating_system/`)

Application code must not depend on or reference files in the `_ai_operating_system/` directory:

- `src/`, `app/`, `lib/`, etc. must be fully independent.
- No code should `require()`, `import`, or reference `_ai_operating_system/` files at runtime.
- `_ai_operating_system/` is an agent operating layer only; it does not ship with the application.

**Enforcement:** Linters and deployment validators MUST flag any code that references `_ai_operating_system/`.
...
1. **Authority source**: `AGENTS.md` and `_ai_operating_system/` exist in the working directory.
2. **No parent-context leakage**: Confirm that planning context comes from local `.cursor/`, `.claude/`, etc., not from parent directories.
3. **Project identity**: Read `_ai_operating_system/.aiast-role.json` to confirm the repo type (downstream-app vs parent-template).

Script: `scripts/check-context-isolation.sh`
...
- `bootstrap/install-aiast.sh` creates local `AGENTS.md`, `_ai_operating_system/`, and tool adapters.
...
- **Read-only inspection** of `_ai_operating_system/`, `AGENTS.md`, and design docs is permitted.
...
| --- | --- | --- | --- |
| Governance (`AGENTS.md`, `_ai_operating_system/`) | Repo-local | Working directory | ❌ No parent fallback |
...
| Application code | Repo-local | Runtime | ❌ Never references `_ai_operating_system/` |
