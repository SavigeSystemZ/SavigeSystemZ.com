...
- **Friction app removal + screenshot collection (2026-06-09):** Set Friction visibility to DRAFT, created `scripts/hide-friction.sh` for automated hiding, collected 58 screenshots from 22 local app repos (AppScope, BlueWraith, BudgetBeacon, CleanoutConnect, CodeSeal, CouplesWealth, DeepWeave, FlipHole, ForgeCouncil, GhostGrid, HQIQ, Immortality, LuxeLogic, ModPilot, Orignym, PromptMage, SiliconLedger, Sipher, SteadyStack, TraceForge, Vetraxis, WisdomWarp), organized in `apps/web/public/showcase/app-media/`, created `docs/APP_VISIBILITY_AND_MEDIA.md` with removal + media workflows. Commit `85ce506` pushed.

- **World-Class completion plan (2026-06-07, landed + pushed):** 52/52 org repos mirrored (`pnpm code:verify-catalog -- --require-ui-catalog`); screenshot tiers (ui-catalog PNG, manual live app UI, GitHub OG); `ApplicationPreviewImage`, `CatalogSearchFilter`, `FeaturedCatalogRunway`, `catalog-enrichment`; admin set-catalog-screenshot; donate lane + mock commerce E2E; `pnpm staging:verify` with HTTP/presign probes; Lighthouse CI job; catalog-coverage E2E (52 slugs). **`scripts/land-catalog-completion.sh`** (8 themed commits) + **`scripts/verify-release.sh`**. **`pnpm verify:release` green (180 Vitest)**; **E2E 86 passed / 1 skipped** (local + GitHub Actions). **5 manual UI captures** (incl. PharmPhreak). **Launch discovery** expanded (`catalog-launch-registry.ts`: LOCAL_DEV_PORTS, PORTS_REGISTRY, case-insensitive MyAppZ paths). **CI fixes:** tracked `pnpm-lock.yaml`, workflow owner secrets, Turbo env passthrough, case-insensitive GitHub webhook sync.
...
- [x] Playwright: archive launch (8), moderation (5), commerce (7+donate), catalog-coverage (52 HTTP), staging-presign, admin-media-catalog, full pipeline (6), a11y (13 routes). **86 passed / 1 skipped** (local + CI).
- [x] Catalog land batch pushed to `origin/main`; GitHub Actions quality/e2e/lighthouse green on `621ac96`.
- [x] Launch URL discovery + `pnpm code:capture-ui-screenshots` / `pnpm code:discover-launches` operator loop documented in `docs/CATALOG_OPERATIONS.md`.
...
**Last updated:** 2026-06-09 (session wrap — P0/P1 complete + Friction/screenshots done)
