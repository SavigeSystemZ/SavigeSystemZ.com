...
**Runtime application code must not depend on or reference the `_ai_operating_system/` directory.**

The `_ai_operating_system/` directory is an **agent operating layer** — it governs how agents build, test, and maintain the application, but it does NOT ship with the deployed application.

## File classification

### Category A: Agent Operating Layer (stays in `_ai_operating_system/`, does not deploy)

These files guide agents and are removed or neutralized before deployment:

- `AGENTS.md` — Agent rules and operating contracts.
- `_ai_operating_system/` — Entire directory with governance, policies, and contracts.
...
These files are the actual application and must NOT reference `_ai_operating_system/`:
...
**Deployment action:** Deploy as-is. These files MUST NOT import or reference `_ai_operating_system/`.
...
**Deployment action:** Review per-application. Remove agent-specific CI (e.g., `scripts/check-*.sh` invocations), keep application-critical CI.
...
1. **Audit runtime code:** Verify no `src/`, `app/`, `lib/` files reference `_ai_operating_system/`.
   ```bash
   grep -r "_ai_operating_system" src/ app/ lib/ --exclude-dir=.git 2>/dev/null && echo "❌ FAIL: Runtime code references _ai_operating_system/" || echo "✓ PASS"
   ```
...
3. **Strip agent operating layer:** Remove or archive:
   - `_ai_operating_system/` directory
...
5. **Test the deployment artifact:** Unpack and verify the application runs without `_ai_operating_system/` present.
...
│
├── [NOT in deployment]
├── _ai_operating_system/                   ← STRIPPED (agent operating layer)
├── AGENTS.md                  ← STRIPPED
...
# Do NOT copy _ai_operating_system/, AGENTS.md, .claude/, etc.
...
tar --exclude='_ai_operating_system' \
    --exclude='.claude' \
    --exclude='.cursor' \
    --exclude='AGENTS.md' \
    --exclude='bootstrap' \
...
tar -tzf my-app-1.0.0.tar.gz | grep -E '_ai_operating_system|AGENTS.md|bootstrap' || echo "✓ Clean"
```
...
- `require('../../_ai_operating_system/...')` or `import _ai_operating_system from '_ai_operating_system'`.
- Reference `AGENTS.md` or operating-layer files.
- Call `scripts/check-*.sh` or agent scripts at runtime.
...
```bash
#!/bin/bash
# Check for runtime code referencing _ai_operating_system
if grep -r "require.*_ai_operating_system\|import.*_ai_operating_system\|from.*_ai_operating_system" \
         src/ app/ lib/ --include="*.js" --include="*.ts" --include="*.py" 2>/dev/null; then
    echo "❌ FAIL: Runtime code references _ai_operating_system/"
    exit 1
else
    echo "✓ PASS: No runtime dependencies on _ai_operating_system/"
fi
```
...
3. The shared `_ai_operating_system/`, `AGENTS.md` stay in the repo only (not deployed).
4. Shared libraries in `lib/`, `packages/` are runtime code (deployed with all apps).

### Development vs. production distributions

- **Development distribution** (for developers): Include `_ai_operating_system/`, `AGENTS.md`, `bootstrap/`, `.claude/`, etc. so devs can use the full toolchain.
- **Production distribution** (for end users/servers): Strip agent operating layer; include only runtime code, docs, and config.

### Configuration management

- `_ai_operating_system/` may contain environment-agnostic operating rules.
...
| --- | --- | --- |
| `src/`, `app/`, `lib/` | ✅ Yes | Runtime code; must not reference `_ai_operating_system/` |
...
| `_ai_operating_system/` | ❌ No | Agent operating layer only |
